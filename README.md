...
```sh
docker pull lpenz/ghaction-cmake:0.15
```

Then, run a container in the project's directory, for instance:

```sh
docker run --rm -t -u "$UID" -w "$PWD" -v "${PWD}:${PWD}" -e INPUT_PRESET=valgrind lpenz/ghaction-cmake:0.15
```
...
jobs:
  include:
    - install: docker pull lpenz/ghaction-cmake:0.15
    - script: docker run --rm -t -u "$UID" -w "$PWD" -v "${PWD}:${PWD}" -e INPUT_PRESET=valgrind lpenz/ghaction-cmake:0.15
```
